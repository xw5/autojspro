...
// 执行跳转
function navigator() {
  if(currentActivity() != "cn.etouch.ecalendar.MainActivity") {
    return;
  }
    ...
    if (nowIndex >= maxTitle) {
        ...
        swipe(device.width/2,device.height-300,device.width/2,300,800);
        //scrollDown();
...
function initSelect() {
    ...
    titles = id("tv_chat_room_tag").find();
...
function scrollDownAction() {
    if(currentActivity() != "cn.etouch.ecalendar.tools.life.LifeDetailsActivity") {
      return;
    }
    if (pageScrollTime < 0) {
        collectClick();
    ...
    sleep(3500);
    //swipe(device.width/2,600,device.width/2,300,500);
    gesture(600, [device.width/2,device.height/2], [device.width/2,300]);
    sleep(1000);
    pageScrollTime --;
    clickMore();
